    ...
    if submitted:
        if not goal.strip():
            ...
            with st.spinner("Проверяем реалистичность параметров..."):
                realism_prompt = f"""Оцени реалистичность запроса строго по двум критериям ниже. Никаких других причин для отказа нет.

- Цель: {goal}
- Бюджет на курсы и учебные материалы: {budget} ₽ в месяц (не зарплата, не доход — именно расходы на обучение)

НЕРЕАЛИСТИЧНО — только если выполнено хотя бы одно из двух условий:
1. Цель физически невозможна для человека ("летать без снаряжения", "стать суперменом", "телепортироваться")
2. Бюджет равен 0 ₽ И тема требует обязательного физического оборудования, которое нельзя получить бесплатно (например, профессиональный музыкальный инструмент, фотокамера, спортивный инвентарь)

Во всех остальных случаях — РЕАЛИСТИЧНО. В частности:
- Любой положительный бюджет всегда достаточен — пользователь сам решает, как его потратить
- Любая реальная профессия или навык — всегда реалистична, даже если путь долгий
- Срок и количество часов не являются основанием для отказа

Если реалистично — верни только слово: РЕАЛИСТИЧНО
Если нет — верни: НЕРЕАЛИСТИЧНО
И на следующей строке одно предложение: укажи конкретно, какой из двух критериев нарушен."""

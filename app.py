...
ВАЖНО: Если цель пользователя не является конкретным навыком или профессией (например "стать суперменом", "стать богатым", "стать лучше") — ОТКАЖИСЬ составлять трек. НЕ пытайся угадать или интерпретировать цель. Напиши ТОЛЬКО: "Пожалуйста, укажи конкретный навык или профессию. Например: Python-разработка, UX-дизайн, английский язык." И больше ничего не добавляй.

...
- Бюджет на оплату курсов и учебных материалов: {budget} ₽ в месяц. Это сумма, которую пользователь готов тратить на обучение — не зарплата, не доход, не стоимость бизнеса. Не конвертируй валюту.
...
2. Бюджет 0 ₽, а тема требует дорогостоящего оборудования или обязательных платных курсов

Если цель достижима — верни только слово: РЕАЛИСТИЧНО
Если нет — верни: НЕРЕАЛИСТИЧНО
И на следующей строке одно предложение: почему именно нереалистично. Бюджет упоминай только в рублях (₽)."""

    ...
    if submitted:
        if not goal.strip():
            ...
            with st.spinner("Проверяем реалистичность параметров..."):
                if budget == 0:
                    realism_prompt = f"""Оцени реалистичность запроса строго по двум критериям. Никаких других причин для отказа нет.

- Цель: {goal}
- Бюджет: 0 ₽ (пользователь хочет учиться только по бесплатным материалам)

НЕРЕАЛИСТИЧНО — только если выполнено хотя бы одно из условий:
1. Цель физически невозможна для человека ("летать без снаряжения", "стать суперменом", "телепортироваться")
2. Тема требует обязательного физического оборудования, которое невозможно получить бесплатно (музыкальный инструмент, фотокамера, спортивный инвентарь и т.п.) — и без него обучение невозможно в принципе

Во всех остальных случаях — РЕАЛИСТИЧНО.

Если реалистично — верни только слово: РЕАЛИСТИЧНО
Если нет — верни: НЕРЕАЛИСТИЧНО
И на следующей строке одно предложение: что конкретно делает цель нереалистичной."""
                else:
                    realism_prompt = f"""Оцени реалистичность цели. Бюджет и сроки НЕ оцениваются — только сама цель.

- Цель: {goal}

НЕРЕАЛИСТИЧНО — только если цель физически невозможна для человека ("летать без снаряжения", "стать суперменом", "телепортироваться").

Во всех остальных случаях — РЕАЛИСТИЧНО. Реальные профессии и навыки всегда реалистичны.

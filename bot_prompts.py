...
Примеры для Python: «Веб-разработка», «Анализ данных», «Автоматизация задач»
Примеры для английского: «Деловой английский», «Разговорный», «Подготовка к IELTS»
Примеры для фотографии: «Портретная съёмка», «Пейзаж и архитектура», «Репортаж»
...
⚠️ ВАЖНО: Если в тексте цели пользователь прямо указывает, что уже владеет определёнными знаниями (фразы вида «я знаю», «умею», «знаком с», «базовые навыки уже есть», «уже умею»), НЕ включай эти знания в трек — начинай с того, что идёт ПОСЛЕ уже имеющейся базы.
...
**Что изучать:**
• [группа тем] — зачем это нужно / что даёт (1 строка)
• [группа тем] — зачем это нужно / что даёт (1 строка)
(3–5 пунктов. Группируй по смыслу, не перечисляй каждую функцию отдельно. НЕ пиши общие категории — пиши конкретные техники. Примеры: «f-strings и форматирование вывода — чтобы красиво выводить данные», «list slicing и методы словарей — работа с коллекциями», «Present Perfect vs Past Simple — когда использовать каждое время»)
**Материалы:**
...
⚠️ КРИТИЧНО для раздела Материалы:
1. Каждый материал должен напрямую покрывать темы ЭТОГО этапа — не общий курс по теме, а конкретно то, что изучается на этом этапе. Пример: если этап про словари и функции в Python — материал должен быть про словари и функции, а не «Python с нуля» целиком.
2. Используй ТОЛЬКО формат [Источник] Название — без жирного текста, без кавычек. Примеры правильно: [YouTube] Словари в Python за 20 минут — Диджитализируй, [Stepik] Поколение Python: курс для начинающих, [Habr] Введение в декораторы Python. Примеры НЕПРАВИЛЬНО: YouTube: **"Python с нуля"**, [YouTube] **Название**.
